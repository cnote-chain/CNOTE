...
#include "qt/c_note/forms/ui_snackbar.h"
#include "qt/c_note/qtutils.h"
#include <QTimer>


SnackBar::SnackBar(C_NoteGUI* _window, QWidget *parent) :
    QDialog(parent),
    ui(new Ui::SnackBar),
    window(_window),
    timeout(MIN_TIMEOUT)
{
    ...
    if (window)
        connect(window, &C_NoteGUI::windowResizeEvent, this, &SnackBar::windowResizeEvent);
    else {
