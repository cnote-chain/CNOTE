...
void OptionsModel::setDisplayDefaultOptions(QSettings& settings, bool reset)
{
    if (!settings.contains("nDisplayUnit") || reset)
        settings.setValue("nDisplayUnit", BitcoinUnits::CNOTE);
    ...
    if (settings.contains("nAnonymizec_noteAmount") || reset)
        gArgs.SoftSetArg("-anonymizec_noteamount", settings.value("nAnonymizec_noteAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in c_note.cpp during shotdown to also remove the window positions
