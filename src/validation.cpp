...
#if defined(NDEBUG)
#error "C_Note cannot be compiled without assertions."
#endif
...
/** Fees smaller than this (in ucnote) are considered zero fee (for relaying, mining and transaction creation)
...
{
    util::ThreadRename("c_note-scriptch");
...
bool AcceptBlock(const CBlock& block, CValidationState& state, CBlockIndex** ppindex, CDiskBlockPos* dbp)
{
    ...
    if (isPoS) {
        ...
        // Inputs
        std::vector<CTxIn> cnoteInputs;

        for (const CTxIn& stakeIn : stakeTxIn.vin) {
            cnoteInputs.push_back(stakeIn);
        }
        const bool hasCNOTEInputs = !cnoteInputs.empty();
        ...
        for (const auto& txIn : block.vtx) {
            ...
            for (const CTxIn& in: tx.vin) {
                ...
                if(hasCNOTEInputs) {
                    ...
                    for (const CTxIn& cnoteIn : cnoteInputs)
                        if (cnoteIn.prevout == in.prevout)
            ...
            while (!chainActive.Contains(prev)) {
                ...
                for (const auto& txIn : bl.vtx) {
                    ...
                    for (const CTxIn& in: t.vin) {
                        ...
                        if (hasCNOTEInputs) {
                            for (const CTxIn& stakeIn : cnoteInputs)
