...
#include "qt/c_note/send.h"
#include "qt/c_note/forms/ui_send.h"
#include "qt/c_note/addnewcontactdialog.h"
#include "qt/c_note/qtutils.h"
#include "qt/c_note/sendchangeaddressdialog.h"
#include "qt/c_note/optionbutton.h"
#include "qt/c_note/sendconfirmdialog.h"
#include "qt/c_note/guitransactionsutils.h"
...
SendWidget::SendWidget(C_NoteGUI* parent) :
    PWidget(parent),
    ui(new Ui::send),
    coinIcon(new QPushButton())
{
    ...
    setCssProperty(coinIcon, "coin-icon-cnote");
    ...
    // Connect
    connect(ui->pushLeft, &QPushButton::clicked, [this](){onCNOTESelected(true);});
    connect(ui->pushRight,  &QPushButton::clicked, [this](){onCNOTESelected(false);});
...
void SendWidget::showHideCheckBoxDelegations(CAmount delegationBalance)
{
    ...
    // coincontrol is not selected, and we are trying to spend transparent CNOTEs.
...
void SendWidget::onShieldCoinsClicked()
{
    ...
    if (availableBalance > 0) {
        ...
        ProcessSend(recipients, true, [this](QList<SendCoinsRecipient>& recipients) {
            ...
            if (!res.result) {
                inform(tr("Error generating address to shield CNOTEs"));
                return false;
            }
        ...
        inform(tr("You don't have any transparent CNOTEs to shield."));
    }
}
...
void SendWidget::onCNOTESelected(bool _isTransparent)
{
