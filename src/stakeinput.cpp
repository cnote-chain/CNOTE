...
CCNoteStake* CCNoteStake::NewCNoteStake(const CTxIn& txin)
{
    ...
    return new CCNoteStake(txPrev->vout[txin.prevout.n],
                         txin.prevout,
                         pindexFrom);
}

bool CCNoteStake::GetTxOutFrom(CTxOut& out) const
{
    out = outputFrom;
    return true;
}

bool CCNoteStake::CreateTxIn(CWallet* pwallet, CTxIn& txIn, uint256 hashTxOut)
{
    txIn = CTxIn(outpointFrom.hash, outpointFrom.n);
    return true;
}

CAmount CCNoteStake::GetValue() const
{
    return outputFrom.nValue;
}

bool CCNoteStake::CreateTxOuts(CWallet* pwallet, std::vector<CTxOut>& vout, CAmount nTotal, const bool onlyP2PK)
{
...
CDataStream CCNoteStake::GetUniqueness() const
{
    //The unique identifier for a CNOTE stake is the outpoint
...
const CBlockIndex* CCNoteStake::GetIndexFrom() const
{
    // Sanity check, pindexFrom is set on the constructor.
    if (!pindexFrom) throw std::runtime_error("CCNoteStake: uninitialized pindexFrom");
    return pindexFrom;
}

// Verify stake contextual checks
bool CCNoteStake::ContextCheck(int nHeight, uint32_t nTime)
{
