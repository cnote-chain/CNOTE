...
- [Development guidelines](#development-guidelines)
    - [General C_Note](#general-c_note-core)
    - [Wallet](#wallet)
    - [General C++](#general-c)
...
C_Note uses [Doxygen](http://www.doxygen.nl/) to generate its official documentation.
...
Run with the `-testnet` option to run with "play CNOTEs (tCNOTE)" on the test network, if you
...
C_Note is a multi-threaded application, and deadlocks or other
...
([`valgrind.supp`](https://github.com/c_note-project/c_note/blob/master/contrib/valgrind.supp))
which includes known Valgrind warnings in our dependencies that cannot be fixed
in-tree. Example use:

```shell
$ valgrind --suppressions=contrib/valgrind.supp src/test/test_c_note
$ valgrind --suppressions=contrib/valgrind.supp --leak-check=full \
      --show-leak-kinds=all src/test/test_c_note --log_level=test_suite
$ valgrind -v --leak-check=full src/c_noted -printtoconsole
```
...
# A coverage report will now be accessible at `./test_c_note.coverage/index.html`.
```
...
- BitcoinMiner : Generates CNOTEs (if wallet is enabled).
...
However, in open source software such as C_Note, where everyone uses
...
pay attention to for reviewers of C_Note code.
...
- For `strprintf`, `LogPrint`, `LogPrintf` formatting characters don't need size specifiers
  ...
  - *Rationale*: C_Note uses tinyformat, which is type safe. Leave them out to avoid confusion
...
be sent upstream but bugfixes may also be prudent to PR against C_Note so that they can be integrated
...
file descriptors it will cause problems with C_Note's `select()` loop, because
...
- When looking at other's pull requests, it may make sense to add the following section to your `.git/config`
  file:
        ...
        [remote "upstream-pull"]
                ...
                url = git@github.com:C_Note-Project/C_Note.git
...
- Try not to overload methods on argument type. E.g. don't make `getblock(true)` and `getblock("hash")`
  ...
  - *Rationale*: This is impossible to use with `c_note-cli`, and can be surprising to users.
...
- Add every non-string RPC argument `(method, idx, name)` to the table `vRPCConvertParams` in `rpc/client.cpp`.
  ...
  - *Rationale*: `c_note-cli` and the GUI debug console use this table to determine how to
