...
void CoinControlDialog::updateLabels()
{
    ...
    ui->labelTitle->setText(fSelectTransparent ?
            "Select CNOTE Outputs to Spend" :
            "Select Shielded CNOTE to Spend");
    ...
    int nDisplayUnit = BitcoinUnits::CNOTE;
                                          ...
                                          const bool isChange)
{
    ...
    // if listMode or change => show C_Note address. In tree mode, address is not shown again for direct wallet address outputs
    if (!treeMode) {
