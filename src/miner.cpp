...
bool ProcessBlockFound(const std::shared_ptr<const CBlock>& pblock, CWallet& wallet, Optional<CReserveKey>& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != g_best_block)
            return error("C_NoteMiner : generated block is stale");
    }
    ...
    if (!ProcessNewBlock(state, nullptr, pblock, nullptr)) {
        return error("C_NoteMiner : ProcessNewBlock, block not accepted");
    }
...
void BitcoinMiner(CWallet* pwallet, bool fProofOfStake)
{
    ...
    util::ThreadRename("c-note-miner");
    ...
    while (fGenerateBitcoins || fProofOfStake) {
        ...
        LogPrintf("Running C_NoteMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
...
void static ThreadBitcoinMiner(void* parg)
{
    ...
    try {
        ...
        LogPrintf("C_NoteMiner exception");
    } catch (...) {
        LogPrintf("C_NoteMiner exception");
    }

    LogPrintf("C_NoteMiner exiting\n");
}
