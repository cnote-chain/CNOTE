...
const char * const C_Note_CONF_FILENAME = "cnote.conf";
const char * const C_Note_PID_FILENAME = "cnote.pid";
const char * const C_Note_MASTERNODE_CONF_FILENAME = "masternode.conf";


// C_Note only features
// Masternode
...
{
// Windows < Vista: C:\Documents and Settings\Username\Application Data\C_Note
// Windows >= Vista: C:\Users\Username\AppData\Roaming\C_Note
// Mac: ~/Library/Application Support/C_Note
// Unix: ~/.c_note
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "CNote";
#else
    fs::path pathRet;
...
static fs::path ZC_GetBaseParamsDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\C_NoteParams
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\C_NoteParams
    // Mac: ~/Library/Application Support/C_NoteParams
    // Unix: ~/.c_note-params
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "CNoteParams";
#else
    fs::path pathRet;
    char* pszHome = getenv("HOME");
    if (pszHome == NULL || strlen(pszHome) == 0)
    ...
    pathRet /= "Library/Application Support";
    TryCreateDirectory(pathRet);
    return pathRet / "CNoteParams";
#else
    // Unix
    return pathRet / ".cnote-params";
#endif
#endif
}
...
void initZKSNARKS()
{
    ...
    if (fs::exists(sapling_spend) && fs::exists(sapling_output)) {
        ...
        sapling_spend = "/usr/share/c_note/sapling-spend.params";
        sapling_output = "/usr/share/c_note/sapling-output.params";
        if (fs::exists(sapling_spend) && fs::exists(sapling_output)) {
            fParamsFound = true;
        } else {
            // Linux fallback for local installs
            sapling_spend = "/usr/local/share/c_note/sapling-spend.params";
            sapling_output = "/usr/local/share/c_note/sapling-output.params";
        }
#endif
        if (fs::exists(sapling_spend) && fs::exists(sapling_output))
            fParamsFound = true;
...
{
    fs::path pathConfigFile(gArgs.GetArg("-conf", C_Note_CONF_FILENAME));
    return AbsPathForConfigVal(pathConfigFile, false);
}

fs::path GetMasternodeConfigFile()
{
    fs::path pathConfigFile(gArgs.GetArg("-mnconf", C_Note_MASTERNODE_CONF_FILENAME));
...
void ArgsManager::ReadConfigFile()
{
    ...
    if (!streamConfig.good()) {
        // Create empty c_note.conf if it does not exist
...

    {
        ...
        for (boost::program_options::detail::config_file_iterator it(streamConfig, setOptions), end; it != end; ++it) {
            // Don't overwrite existing settings so command line settings override c_note.conf
...
fs::path GetPidFile()
{
    fs::path pathPidFile(gArgs.GetArg("-pid", C_Note_PID_FILENAME));
