...
#include "qt/c_note/dashboardwidget.h"
#include "clientmodel.h"
#include "guiutil.h"
#include "optionsmodel.h"
#include "qt/c_note/forms/ui_dashboardwidget.h"
#include "qt/c_note/qtutils.h"
#include "qt/c_note/sendconfirmdialog.h"
...
DashboardWidget::DashboardWidget(C_NoteGUI* parent) : PWidget(parent),
                                                      ui(new Ui::DashboardWidget)
{
    ...
    setCssProperty(ui->labelSquareCNote, "square-chart-cnote");
    setCssProperty(ui->labelSquareMN, "square-chart-mn");
    setCssProperty(ui->labelCNote, "text-chart-cnote");
    ...
    setCssProperty(ui->labelAmountCNote, "text-stake-cnote-disable");
    ...
    if (window)
        connect(window, &C_NoteGUI::windowResizeEvent, this, &DashboardWidget::windowResizeEvent);
#endif
...
bool DashboardWidget::loadChartData(bool withMonthNames)
{
    ...
    chartData->amountsByCache = getAmountBy(); // pair CNOTE
    ...
    for (int j = range.first; j < range.second; j++) {
        ...
        chartData->valuesCNote.append(cnote);
        chartData->valuesMN.append(mn);

        int max = std::max(cnote, mn);
...
void DashboardWidget::onChartRefreshed()
{
    ...
    set0->setColor(QColor(209, 158, 21));
    set1->setColor(QColor(243, 179, 7));
    ...
    set0->append(chartData->valuesCNote);
    set1->append(chartData->valuesMN);

    // Total
    nDisplayUnit = walletModel->getOptionsModel()->getDisplayUnit();
    if (chartData->totalCNote > 0 || chartData->totalMN > 0) {
        setCssProperty(ui->labelAmountCNote, "text-stake-cnote");
        setCssProperty(ui->labelAmountMN, "text-stake-mn");
    } else {
        setCssProperty(ui->labelAmountCNote, "text-stake-cnote-disable");
        setCssProperty(ui->labelAmountMN, "text-stake-mn-disable");
    }
    forceUpdateStyle({ui->labelAmountCNote, ui->labelAmountMN});
    ui->labelAmountCNote->setText(GUIUtil::formatBalance(chartData->totalCNote, nDisplayUnit));
