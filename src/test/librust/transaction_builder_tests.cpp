...
BOOST_AUTO_TEST_CASE(TransparentToSapling)
{
    ...
    // 0.5 t-CNOTE in, 0.4 shielded-CNOTE out, 0.1 t-CNOTE fee
...
BOOST_AUTO_TEST_CASE(SaplingToSapling) {
    ...
    // --- 0.4 shielded-CNOTE in, 0.299 shielded-CNOTE out, 0.1 shielded-CNOTE fee, 0.001 shielded-CNOTE change (added to fee)
    ...
    // --- Now try with 1 shielded-CNOTE in, 0.5 shielded-CNOTE out, 0.1 shielded-CNOTE fee, 0.4 shielded-CNOTE change
...
BOOST_AUTO_TEST_CASE(FailsWithNegativeChange)
{
    ...
    // Generate a 0.5 CNOTE note
    auto testNote = GetTestSaplingNote(pa, 59990000);

    // Fail if there is only a Sapling output
    // 0.5 shielded-CNOTE out, 0.1 t-CNOTE fee
    auto builder = TransactionBuilder(consensusParams, 1);
    ...
    // 0.5 t-CNOTE out, 0.1 t-CNOTE fee
    ...
    // 0.5 t-CNOTE out, 0.1 t-CNOTE fee, 0.59999 shielded-CNOTE in
    builder.AddSaplingSpend(expsk, testNote.note, testNote.tree.root(), testNote.tree.witness());
    BOOST_CHECK_EQUAL("Change cannot be negative", builder.Build().GetError());
