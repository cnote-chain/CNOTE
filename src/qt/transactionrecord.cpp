...
bool TransactionRecord::decomposeCoinStake(const CWallet* wallet, const CWalletTx& wtx, const CAmount& nCredit, const CAmount& nDebit, QList<TransactionRecord>& parts)
{
    ...
    bool alreadyHasMNReward = false;
    for (const TransactionRecord& r : parts) {
        if (r.type == TransactionRecord::MNReward) {
            alreadyHasMNReward = true;
            break;
        }
    }
    int nIndexDev = wtx.tx->vout.size() - 1;
    CTxDestination destDev = DecodeDestination(Params().getDevRewardAddress(chainActive.Tip()->nHeight));
    for (int i = 1; i < wtx.tx->vout.size(); ++i) {
        if (i == nIndexDev) continue; // skip dev reward
        CTxDestination destMN;
        bool extracted = ExtractDestination(wtx.tx->vout[i].scriptPubKey, destMN);
        isminetype mnMine = extracted ? wallet->IsMine(wtx.tx->vout[i]) : ISMINE_NO;
        if (extracted && mnMine) {
            TransactionRecord mnRewardSub(hash, wtx.GetTxTime(), wtx.tx->GetTotalSize());
            mnRewardSub.involvesWatchAddress = mnMine & ISMINE_WATCH_ONLY;
            mnRewardSub.type = TransactionRecord::MNReward;
            mnRewardSub.address = EncodeDestination(destMN);
            mnRewardSub.credit = wtx.tx->vout[i].nValue;
            parts.append(mnRewardSub);
        }
    }
    // Dev reward (unchanged)
    CTxDestination dest;
    isminetype mine;
    if (ExtractDestination(wtx.tx->vout[nIndexDev].scriptPubKey, dest) && (mine = IsMine(*wallet, destDev)) && dest == destDev) {
            TransactionRecord devRewardSub(hash, wtx.GetTxTime(), wtx.tx->GetTotalSize());
            devRewardSub.involvesWatchAddress = mine & ISMINE_WATCH_ONLY;
            devRewardSub.type = TransactionRecord::DevReward;
            devRewardSub.address = EncodeDestination(destDev);
            devRewardSub.credit = wtx.tx->vout[nIndexDev].nValue;
            parts.append(devRewardSub);
        }
        return true;
}
...
bool TransactionRecord::decomposeCreditTransaction(const CWallet* wallet, const CWalletTx& wtx, QList<TransactionRecord>& parts)
{
    ...
    for (unsigned int nOut = 0; nOut < wtx.tx->vout.size(); nOut++) {
        ...
        if (mine) {
            // Always append, regardless of txout.nValue size (show all outputs)
            CTxDestination address;
            sub.idx = (int) nOut; // vout index
            sub.credit = txout.nValue;
            sub.involvesWatchAddress = mine & ISMINE_WATCH_ONLY;
            if (ExtractDestination(txout.scriptPubKey, address) && IsMine(*wallet, address)) {
                // Received by C_Note Address
            ...
            // No filter for small nValue here!
            parts.append(sub);
        }
    }
    ...
    for (unsigned int nOut = 0; nOut < wtx.tx->vout.size(); nOut++) {
        ...
        if (ExtractDestination(txout.scriptPubKey, address)) {
            ...
            if (sub.address.empty() && txout.scriptPubKey.StartsWithOpcode(OP_RETURN)) {
                ...
                // Burned CNOTEs, op_return could be for a proposal/budget fee or another sort of data stored there.
