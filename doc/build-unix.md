UNIX BUILD NOTES
====================
Some notes on how to build C_Note in Unix.

Note
---------------------
Always use absolute paths to configure and compile C_Note and the dependencies,
...
memory available when compiling C_Note. On systems with less, gcc can be
 ...
 **For Ubuntu only:** db4.8 packages are available [here](https://launchpad.net/~c_note/+archive/c_note).
 You can add the repository using the following command:

    sudo apt-get install software-properties-common
    sudo add-apt-repository ppa:c_note/c_note
    sudo apt-get update
...
To build C_Note without wallet, see [*Disable-wallet mode*](/doc/build-unix.md#disable-wallet-mode)
...
If you want to build c_note-qt, make sure that the required packages for Qt development
...
Once these are installed, they will be found by configure and a c_note-qt executable will be
built by default.
...
Notes
-----
The release is built with GCC and then "strip c_noted" to strip the debug
...
Security
--------
To help make your C_Note installation more secure by making certain attacks impossible to
...
* _Non-executable Stack_: If the stack is executable then trivial stack-based buffer overflow exploits are possible if
    vulnerable buffers are found. By default, C_Note should be built with a non-executable stack
...
When the intention is to run only a P2P node without a wallet, C_Note may be compiled in
