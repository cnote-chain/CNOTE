...
#include "qt/c_note/settings/forms/ui_settingsinformationwidget.h"
...
SettingsInformationWidget::SettingsInformationWidget(C_NoteGUI* _window,QWidget *parent) :
    ...
    ui(new Ui::SettingsInformationWidget)
{
    ...
    connect(ui->pushButtonFile, &QPushButton::clicked, [this](){
        if (!GUIUtil::openConfigfile())
            inform(tr("Unable to open c_note.conf with default application"));
    });
