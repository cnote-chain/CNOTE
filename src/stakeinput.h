...
#ifndef C_Note_STAKEINPUT_H
#define C_Note_STAKEINPUT_H
...
class CCNoteStake : public CStakeInput
{
private:
...
public:
    CCNoteStake(const CTxOut& _from, const COutPoint& _outPointFrom, const CBlockIndex* _pindexFrom) :
            CStakeInput(_pindexFrom), outputFrom(_from), outpointFrom(_outPointFrom) {}

    static CCNoteStake* NewCNoteStake(const CTxIn& txin);
