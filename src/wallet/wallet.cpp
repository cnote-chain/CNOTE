...
/**
 * Fees smaller than this (in ucnote) are considered zero fee (for transaction creation)
...
bool CWallet::GetMasternodeVinAndKeys(CTxIn& txinRet, CPubKey& pubKeyRet, CKey& keyRet, std::string strTxHash, std::string strOutputIndex, std::string& strError)
{
    ...
    if (txOut.nValue != GetMNCollateral(chainActive.Height()) * COIN) {
        strError = "Invalid collateral tx value, must be Collateral Amount of C-NOTE";
        ...
        LOCK2(cs_main, cs_wallet);
        {
            ...
            while (true) {
                ...
                if (nChange > 0) {
                    ...
                    // change transaction isn't always pay-to-c_note-address
        ...
        std::vector<CStakeableOutput>* availableCoins)
{
    ...
    for (auto it = availableCoins->begin(); it != availableCoins->end();) {
        ...
        CCNoteStake stakeInput(it->tx->tx->vout[it->i],
                             outPoint,
                             it->pindex);
...
std::string CWallet::GetWalletHelpString(bool showDebug)
{
    ...
    strUsage += HelpMessageOpt("-minstakesplit=<amt>", strprintf(_("Minimum positive amount (in CNOTE) allowed by GUI and RPC for the stake split threshold (default: %s)"), FormatMoney(DEFAULT_MIN_STAKE_SPLIT_THRESHOLD)));
...
CWallet* CWallet::CreateWalletFromFile(const std::string walletFile)
{
    ...
    if (nLoadWalletRet != DB_LOAD_OK) {
        if (nLoadWalletRet == DB_CORRUPT) {
            ...
            UIError(strprintf(_("Error loading %s: Wallet requires newer version of C_Note"), walletFile));
            return nullptr;
        } else if (nLoadWalletRet == DB_NEED_REWRITE) {
            UIError(_("Wallet needed to be rewritten: restart C_Note to complete"));
            return nullptr;
        } else {
    ...
    if (gArgs.GetBoolArg("-upgradewallet", fFirstRun && !fLegacyWallet) ||
        ...
        if (prev_version <= FEATURE_PRE_C_Note && walletInstance->IsLocked()) {
    ...
    if (fFirstRun) {
        if (!fLegacyWallet) {
            ...
            walletInstance->SetMaxVersion(FEATURE_PRE_C_Note);
        }
