...
UniValue importprivkey(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() < 1 || request.params.size() > 4)
        throw std::runtime_error(
            "importprivkey \"c_noteprivkey\" ( \"label\" rescan fStakingAddress )\n"
            "\nAdds a private key (as returned by dumpprivkey) to your wallet.\n" +
            HelpRequiringPassphrase() + "\n"

            "\nArguments:\n"
            "1. \"c_noteprivkey\"      (string, required) The private key (see dumpprivkey)\n"
    ...
    CKeyID vchAddress = pubkey.GetID();
    {
        ...
        if (fRescan) {
            ...
            if (fStakingAddress && !Params().IsRegTestNet()) {
                // cold staking was activated after nBlockTimeProtocolV2 (C_Note v4.0). No need to scan the whole chain
...
UniValue importaddress(const JSONRPCRequest& request)
{
    ...
    if (IsValidDestination(dest)) {
        ...
        throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid C-Note address or script");
    }
...
UniValue importwallet(const JSONRPCRequest& request)
{
    ...
    while (file.good()) {
        ...
        // Sapling keys
        // Let's see if the address is a valid C_Note spending key
...
UniValue dumpprivkey(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 1)
        throw std::runtime_error(
            "dumpprivkey \"c_noteaddress\"\n"
            "\nReveals the private key corresponding to 'c_noteaddress'.\n"
            "Then the importprivkey can be used with this output\n" +
            HelpRequiringPassphrase() + "\n"

            "\nArguments:\n"
            "1. \"c_noteaddress\"   (string, required) The c_note address for the private key\n"
...
UniValue bip38encrypt(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 2)
        throw std::runtime_error(
            "bip38encrypt \"c_noteaddress\" \"passphrase\"\n"
            "\nEncrypts a private key corresponding to 'c_noteaddress'.\n" +
            HelpRequiringPassphrase() + "\n"

            "\nArguments:\n"
            "1. \"c_noteaddress\"   (string, required) The cnote address for the private key (you must hold the key already)\n"
...
UniValue bip38decrypt(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 2)
        throw std::runtime_error(
            "bip38decrypt \"c_noteaddress\" \"passphrase\"\n"
